...
void Cpp_Picture(ParameterPicture parameter_picture, unsigned char *data)
{
...
extern "C" cudaError_t RUN_CUDA(long no_tuile, long nb_tuiles, unsigned char *datas)
{
...
// Fonction pour exécuter le kernel CUDA
extern "C" void RUN_CPP(long no_tuile, long nb_tuiles, unsigned char *datas)
{
...
//    size_t size = parameter_picture.get_size_array_2D_tuile() * sizeof(unsigned char);
  //  unsigned char *dev_datas = 0;
    ///dev_datas = (unsigned char *)malloc(size);
    Cpp_Picture(parameter_picture,datas);
}
int main()
{
    std::string json_file = "config.json";
    ...
    cudaStatus = RUN_CUDA(0, 2, datas_G);
    std::cout << "fin du RUN Cuda" << std::endl;

    std::cout << "début du RUN CPP" << std::endl;
    RUN_CPP(0, 2, datas_G);
    std::cout << "fin du RUN CPP" << std::endl;
}